...
def test_field_does_not_exist():
    with pytest.raises(FieldDoesNotExist):
        Expense(amounta=200, pay_method=PayMethods.objects().first(), timestamp=datetime.datetime.utcnow, category=Categories.objects().first()).save()
...
def test_save():
    ...
    Expense(amount=200, pay_method=pay_method, timestamp=datetime.datetime.utcnow,
