...
def test_field_does_not_exist():
    with pytest.raises(FieldDoesNotExist):
        Expense(amounta=200, pay_method=PayMethods.objects().first(), timestamp=datetime.utcnow,
                category=Categories.objects().first(), work_group=WorkGroups.objects().first()).save()


def test_missing_required_field():
    with pytest.raises(ValidationError):
        Expense(amount=200, pay_method=PayMethods.objects().first(), category=Categories.objects().first(),
...
def test_save():
    ...
    Expense(amount=200, pay_method=pay_method, timestamp=datetime.utcnow, category=category,
